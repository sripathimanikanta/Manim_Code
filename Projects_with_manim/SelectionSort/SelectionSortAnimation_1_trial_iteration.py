...
class SelectionSortAnime(Scene):
	"""docstring for SelectionSortAnime"""
	def construct(self):
		...
		keySquare = Square(side_length=sl).next_to(vg[0][0].get_center(), DOWN*5)
		key = Integer(0)
		...
		self.play(
			...
			vg[0][0].animate.set_fill("MAROON", opacity=0)
			)
		self.play(
			vg[0][0].animate.set_fill("MAROON", opacity=0.5),
			vg[1][0].animate.set_fill("RED", opacity=0.5)
			)
